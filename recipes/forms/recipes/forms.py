...
class RecipeForm(ModelForm):
        ...
        def __init__(self, data=None, instance=None, *args, **kwargs):
            ...
            # _initial = {}
            # ##extra_iq is a list of 4-tuples(ingredient,quantity,uom,cat)
            extra_iq = kwargs.pop('extra_iq', [])

            extra_step = kwargs.pop('extra_step', [])
            var2 = len(extra_step)
            ...
            if data is not None:
                if extra_iq:
                    ...
                    i = self.find_count_of_ing(data)         # #new count of ingredients fields less than instance

            steps = instance.get_instructions()
            var3 = len(steps)
            if data is None:
            ...
            # #For ingredients and quantities fields for viewing recipes
            ...
            for i, field in enumerate(extra_iq, i or 1):
            ...
            uom_form_field = fields_for_model(Ingredient, ['uom'])
            category_form_field = fields_for_model(Ingredient, ['category'])
            for j in range(i):
                # value =  Ingredient.objects.get(name =_initial['ingredient%d'%j])
                b["ingredient"] = ModelChoiceField(queryset=Ingredient.objects.all(), empty_label=None,
                                                   to_field_name='name', required=True)
                self.fields.update({
                    'ingredient%d' % j: b["ingredient"],
                    'quantity%d' % j: b["quantity"],
                    'uom%d' % j: uom_form_field['uom'],
                    'category%d' % j: category_form_field['category']
                })
        ...
        # #For instructions fields
            ...
            if extra_step:
                for k, step in enumerate(extra_step, var3):
                    data.update({'instructions_blob_%d' % k: step})
        ...
        def save(self, *args, **kwargs):
            ...
            count = self.find_count_of_ing(self.cleaned_data)
            ingredients_dic = {}
            ...
            for i in range(count):
                ...
                if isinstance(self.cleaned_data['ingredient%d' % i], str):
                    ingredients_dic.update({
                        self.cleaned_data['ingredient%d' % i]: (self.cleaned_data['category%d' % i],
                                                                n, self.cleaned_data['uom%d' % i])
                    })
                else:
                    ingredients_dic.update({
        ...
        #         ,
        #
        #             "instructions_blob_0":"Place mint leaves in the bottom of the glass,
